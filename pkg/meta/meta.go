...
import (
	...
	"github.com/zhihu/zetta/pkg/structure"
	"go.uber.org/zap"
)
...
var (
	mMetaPrefix       = codec.EncodeInt([]byte{}, int64(0))
...
const reservedIDLength = 10 // reserve id<10 for special purpose.
...
func (m *Meta) GenGlobalID() (int64, error) {
	...
	id, err := m.txn.Inc(mNextGlobalIDKey, 1)
	return id + reservedIDLength, err
}

// GetGlobalID gets current global id.
func (m *Meta) GetGlobalID() (int64, error) {
	id, err := m.txn.GetInt64(mNextGlobalIDKey)
	return id + reservedIDLength, err
}
...
func (m *Meta) CreateDatabase(dbMeta *model.DatabaseMeta) error {
	dbKey := m.dbKey(dbMeta.Id)

	if err := m.checkDBNotExists(dbKey); err != nil {
		return errors.Trace(err)
	}

	data, err := json.Marshal(dbMeta)
	if err != nil {
		return errors.Trace(err)
	}
...
func (m *Meta) UpdateDatabase(dbMeta *model.DatabaseMeta) error {
	dbKey := m.dbKey(dbMeta.Id)
...
func (m *Meta) CreateTableOrView(dbID int64, tableMeta *model.TableMeta) error {
	// Check if db exists.
	dbKey := m.dbKey(dbID)
	if err := m.checkDBExists(dbKey); err != nil {
		return errors.Trace(err)
	...
	// Check if table exists.
	tableKey := m.tableKey(tableMeta.Id)
	if err := m.checkTableNotExists(dbKey, tableKey); err != nil {
		return errors.Trace(err)
	}

	data, err := json.Marshal(tableMeta)
	if err != nil {
		return errors.Trace(err)
	}
...
func (m *Meta) CreateTableAndSetAutoID(dbID int64, tableMeta *model.TableMeta, autoID int64) error {
	err := m.CreateTableOrView(dbID, tableMeta)
	if err != nil {
		return errors.Trace(err)
	}
	_, err = m.txn.HInc(m.dbKey(dbID), m.autoTableIDKey(tableMeta.Id), autoID)
	return errors.Trace(err)
}
...
func (m *Meta) UpdateTable(dbID int64, tableMeta *model.TableMeta) error {
	// Check if db exists.
	dbKey := m.dbKey(dbID)
	if err := m.checkDBExists(dbKey); err != nil {
		return errors.Trace(err)
...
func (m *Meta) ListTables(dbID int64) ([]*model.TableMeta, error) {
	dbKey := m.dbKey(dbID)
	if err := m.checkDBExists(dbKey); err != nil {
	...
	tables := make([]*model.TableMeta, 0, len(res)/2)
	for _, r := range res {
		...
		err = json.Unmarshal(r.Value, tbMeta)
		if err != nil {
			return nil, errors.Trace(err)
		}

		tables = append(tables, tbMeta)
	}
...
func (m *Meta) ListDatabases() ([]*model.DatabaseMeta, error) {
	res, err := m.txn.HGetAll(mDBs)
	if err != nil {
		return nil, errors.Trace(err)
	}

	dbs := make([]*model.DatabaseMeta, 0, len(res))
	for _, r := range res {
...
func (m *Meta) GetDatabase(dbID int64) (*model.DatabaseMeta, error) {
	dbKey := m.dbKey(dbID)
...
func (m *Meta) GetTable(dbID int64, tableID int64) (*model.TableMeta, error) {
	// Check if db exists.
	dbKey := m.dbKey(dbID)
	if err := m.checkDBExists(dbKey); err != nil {
		return nil, errors.Trace(err)
	...
	err = json.Unmarshal(value, tableMeta)
	return tableMeta, errors.Trace(err)
}
...
func (m *Meta) UpdateDDLReorgHandle(job *model.Job, startHandle, endHandle []byte, physicalTableID int64) error {
	err := m.txn.HSet(mDDLJobReorgKey, m.reorgJobStartHandle(job.ID), startHandle)
	if err != nil {
		return errors.Trace(err)
	}
	err = m.txn.HSet(mDDLJobReorgKey, m.reorgJobEndHandle(job.ID), endHandle)
	if err != nil {
		return errors.Trace(err)
	}
...
func (m *Meta) GetDDLReorgHandle(job *model.Job) (startHandle, endHandle []byte, physicalTableID int64, err error) {
	startHandle, err = m.txn.HGet(mDDLJobReorgKey, m.reorgJobStartHandle(job.ID))

	if err != nil {
		err = errors.Trace(err)
		return
	}
	endHandle, err = m.txn.HGet(mDDLJobReorgKey, m.reorgJobEndHandle(job.ID))
	if err != nil {
		err = errors.Trace(err)
		return
	}
