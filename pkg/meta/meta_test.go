...
func (s *testSuite) TestMeta(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(v, DeepEquals, dbMeta)

	dbMeta.Database = "aa"
	err = t.UpdateDatabase(dbMeta)
	c.Assert(err, IsNil)
	...
	c.Assert(err, IsNil)
	c.Assert(dbs, DeepEquals, []*model.DatabaseMeta{dbMeta})
	...
	err = t.CreateTableOrView(1, tbMeta)
	c.Assert(err, IsNil)
	...
	tbMeta.TableName = "tt"
	err = t.UpdateTable(1, tbMeta)
	c.Assert(err, IsNil)

	table, err := t.GetTable(1, 1)
	c.Assert(err, IsNil)
	c.Assert(table, DeepEquals, tbMeta)

	table, err = t.GetTable(1, 2)
	c.Assert(err, IsNil)
	c.Assert(table, IsNil)

	tbMeta2 := &model.TableMeta{
		TableMeta: tspb.TableMeta{
			Id:        2,
			TableName: "bb",
		},
	}
	err = t.CreateTableOrView(1, tbMeta2)
	c.Assert(err, IsNil)

	tables, err := t.ListTables(1)
	c.Assert(err, IsNil)
	c.Assert(tables, DeepEquals, []*model.TableMeta{tbMeta, tbMeta2})
	// Generate an auto id.
	...
	err = t.DropTableOrView(1, tbMeta2.Id, true)
	c.Assert(err, IsNil)
	...
	tables, err = t.ListTables(1)
	c.Assert(err, IsNil)
	c.Assert(tables, DeepEquals, []*model.TableMeta{tbMeta})

	// Test case for drop a table without delete auto id key-value entry.
	tid := int64(100)
	tbMeta100 := &model.TableMeta{
		TableMeta: tspb.TableMeta{
			Id:        tid,
			TableName: "t_rename",
		},
	...
	// Create table.
	err = t.CreateTableOrView(1, tbMeta100)
	c.Assert(err, IsNil)
	// Update auto ID.
	currentDBID := int64(1)
	...
	tbMeta3 := &model.TableMeta{
		TableMeta: tspb.TableMeta{
			Id:        3,
			TableName: "tbl3",
		},
	}
	err = t.CreateTableAndSetAutoID(1, tbMeta3, 123)
	c.Assert(err, IsNil)
	id, err := t.GetAutoTableID(1, tbMeta3.Id)
	c.Assert(err, IsNil)
	c.Assert(id, Equals, int64(123))
	// Test case for GenAutoTableIDKeyValue.
	key, val := t.GenAutoTableIDKeyValue(1, tbMeta3.Id, 1234)
	c.Assert(val, DeepEquals, []byte(strconv.FormatInt(1234, 10)))
	c.Assert(key, DeepEquals, []byte{0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x44, 0x42, 0x3a, 0x31, 0x0, 0x0, 0x0, 0x0,
		0x4, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x68, 0x54, 0x49, 0x44, 0x3a, 0x33, 0x0, 0x0, 0x0, 0x5})
	...
	c.Assert(key, DeepEquals, []byte{0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x44, 0x44, 0x4c, 0x4a, 0x6f, 0x62, 0x48, 0x69, 0x9,
		0x73, 0x74, 0x6f, 0x72, 0x79, 0x0, 0x0, 0x0, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x68, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3, 0x78, 0x8})
}
