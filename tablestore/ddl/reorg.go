...
// start and end handle are all encoded keys.
func getTableRange(d *ddlCtx, tbl table.PhysicalTable, snapshotVer uint64, priority int) (startKey, endKey kv.Key, err error) {
	startKey = tbl.RecordPrefix()
	endKey = startKey.PrefixNext()
	// Get the start handle of this partition.
	// Tranverse from the very first key, only including the table prefix which is not existing.When found the
	// first exsiting key with the table prfix, this key is the first key in the table.
	err = iterateSnapshotRows(d.store, priority, tbl, snapshotVer, startKey, endKey, false, false,
		func(h []byte, rowKey kv.Key, rawRecord []byte) (bool, error) {
			startKey = rowKey
			return false, nil
		})
	if err != nil {
	...
	err = iterateSnapshotRows(d.store, priority, tbl, snapshotVer, startKey, endKey, false, true,
		func(h []byte, rowKey kv.Key, rawRecord []byte) (bool, error) {
			endKey = rowKey
			return false, nil
		})
...
func getReorgInfo(d *ddlCtx, t *meta.Meta, job *model.Job, tbl table.Table) (*reorgInfo, error) {
	...
	if job.SnapshotVer == 0 {
		...
		logutil.Logger(ddlLogCtx).Info("[ddl] job get table range", zap.Int64("jobID", job.ID),
			zap.Int64("physicalTableID", pid), zap.ByteString("startHandle", start), zap.ByteString("endHandle", end))
