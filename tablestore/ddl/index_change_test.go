...
func (s *testIndexChangeSuite) TestSmallFunctions(c *C) {
	d := testNewDDL(context.Background(), nil, s.store, nil, nil, testLease)
	defer d.Stop()
	// create table t (c1 int primary key, c2 int);
	tblInfo := testTableInfo(c, d, "t", 2)
	tblInfo.PrimaryKey = []string{"c1"}
	ctx := testNewContext(d)
	err := ctx.NewTxn(context.Background())
	c.Assert(err, IsNil)
	testCreateTable(c, ctx, d, s.dbInfo, tblInfo)
	originTable := testGetTable(c, d, s.dbInfo.Id, tblInfo.Id)
	ptbl := originTable.(table.PhysicalTable)

	c.Assert(err, IsNil)
	// Values for one row
	valueOneRow := tspb.ListValue{
		Values: []*tspb.Value{
			{Kind: &tspb.Value_StringValue{StringValue: "1"}},
			{Kind: &tspb.Value_StringValue{StringValue: "2"}},
		},
	}
	valueOneRow1 := tspb.ListValue{
		Values: []*tspb.Value{
			{Kind: &tspb.Value_StringValue{StringValue: "2"}},
			{Kind: &tspb.Value_StringValue{StringValue: "3"}},
		},
	}
	valueOneRow2 := tspb.ListValue{
		Values: []*tspb.Value{
			{Kind: &tspb.Value_StringValue{StringValue: "3"}},
			{Kind: &tspb.Value_StringValue{StringValue: "4"}},
		},
	}
	txn, err := d.GetStore().Begin()
	c.Assert(err, IsNil)
	ctx1 := context.WithValue(context.Background(), sessionctx.TxnIDKey, txn)
	err = originTable.Insert(ctx1, &tspb.KeySet{}, "", []string{"c1", "c2"}, []*tspb.ListValue{&valueOneRow})
	c.Assert(err, IsNil)
	err = originTable.Insert(ctx1, &tspb.KeySet{}, "", []string{"c1", "c2"}, []*tspb.ListValue{&valueOneRow1})
	c.Assert(err, IsNil)
	err = originTable.Insert(ctx1, &tspb.KeySet{}, "", []string{"c1", "c2"}, []*tspb.ListValue{&valueOneRow2})
	c.Assert(err, IsNil)
	err = txn.Commit(context.Background())
	c.Assert(err, IsNil)
	ver, _ := s.store.CurrentVersion()
	_, _, err = getTableRange(d.ddlCtx, ptbl, ver.Ver, 1)
	c.Assert(err, IsNil)
...
func (s *testIndexChangeSuite) TestIndexChange(c *C) {
	...
	idxmeta := &model.IndexMeta{
		IndexMeta: tspb.IndexMeta{
			Name:           "test",
			TableName:      "t",
			DefinedColumns: []string{"c2"},
		},
	}
	testCreateIndex(c, ctx, d, 1, 11, idxmeta)
}
