...
func (s *testIndexChangeSuite) TestSmallFunctions(c *C) {
	...
	err = originTable.Insert(ctx1, []string{"c1", "c2"}, []*tspb.ListValue{&valueOneRow})
	c.Assert(err, IsNil)
	err = originTable.Insert(ctx1, []string{"c1", "c2"}, []*tspb.ListValue{&valueOneRow1})
	c.Assert(err, IsNil)
	err = originTable.Insert(ctx1, []string{"c1", "c2"}, []*tspb.ListValue{&valueOneRow2})
	c.Assert(err, IsNil)
